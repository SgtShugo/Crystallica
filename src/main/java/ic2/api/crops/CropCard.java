 ...
 * 
 * Any crop extending this can be registered using registerCrop to be added into
 * the game.
 */
public abstract class CropCard {
	...
	/**
	 * Description of your plant. Keep it short, a few characters per line for
	 * up to two lines. Default is showing attributes of your plant, 2 per line.
	 * 
	 * @param i
	 *            line to get, starting from 0
	 * @return The line
	 */
	...
	/**
	 * Tier of the plant. Ranges from 1 to 16, 0 is Weed. Valuable and powerful
	 * crops have higher tiers, useless and weak ones have lower tiers.
	 * 
	 * @return Tier
	 */
	...
	/**
	 * Describe the plant through a set of stats, influencing breeding. Plants
	 * sharing stats and attributes will tend to cross-breed more often.
	 * 
	 ...
	 * 
	 * @param n
	 *            index of the requested stat
	 * @return The requested value of the stats
	 */
	public abstract int stat(int n);

	/**
	 * Additional attributes of the plant, also influencing breeding. Plants
	 * sharing stats and attributes will tend to cross-breed more often.
	 * 
	...
	public void registerSprites(IIconRegister iconRegister) {
		...
		for (int i = 1; i <= textures.length; i++) {
			textures[i - 1] = iconRegister.registerIcon("ic2:crop/blockCrop."
					+ name() + "." + i);
		}
	}
	...
	/**
	 * Amount of growth points needed to increase the plant's size. Default is
	 * 200 * tier.
	 */
	 ...
	 * 
	 * Consider: - Humidity, nutrients and air quality - Current size - Light
	 * level - Special biomes or conditions, accessible through crop.worldObj
	 * 
	 * This method will be called upon empty upgraded crops to check whether a
	 * neighboring plant can cross onto it! Don't check if the size is greater
	 * than 0 and if the ID is real.
	 * 
	 * @param crop
	 *            reference to ICropTile
	...
	/**
	 * Calculate the influence for the plant to grow based on humidity,
	 * nutrients and air. Normal behavior is rating the three stats "normal",
	 * with each of them ranging from 0-30. Basic rule: Assume everything
	 * returns 10. All together must equal 30. Add the factors to your likings,
	 ...
	 *            reference to ICropTile
	 * @param humidity
	 *            ground humidity, influenced by hydration
	 * @param nutrients
	 *            nutrient quality in ground, based on fertilizers
	 * @param air
	 *            air quality, influences by open gardens and less crops
	...
	/**
	 * Called when the plant is rightclicked by a player. Default action is
	 * harvesting.
	 * 
	...
	/**
	 * Base chance for dropping the plant's gains, specify values greater than 1
	 * for multiple drops. Default is 0.95^tier.
	 * 
	...
	/**
	 * Called when the plant is leftclicked by a player. Default action is
	 * picking the plant.
	 * 
	 * Only called Serverside.
	 * 
	...
	/**
	 * Base chance for dropping seeds when the plant is picked. Default is
	 * 0.5*0.8^tier with a bigger chance for sizes greater than 2 and absolutely
	 * no chance for size 0.
	 * 
	 * @param crop
	 *            reference to ICropTile
	...
	/**
	 * Obtain seeds dropped when the plant is picked. Multiple drops of the
	 * returned ItemStack can occur. Default action is generating a seed from
	 * this crop.
	 * 
	 * @param crop
	 *            reference to ICropTile
	...
	/**
	 * Default is true if the entity is an EntityLiving in jumping or sprinting
	 * state.
	 * 
	 * @param crop
	 *            reference to ICropTile
	 * @param entity
	 *            entity colliding
	 * @return Whether trampling calculation should happen, return false if the
	 *         plant is no longer valid.
	 */
	...
	/**
	 * Called every time the crop ticks. Should be called every 256 ticks or
	 * around 13 seconds.
	 * 
	 * @param crop
	 *            reference to ICropTile
	...
	/**
	 * Check whether this plant spreads weed to surrounding tiles. Default is
	 * true if the plant has a high growth stat (or is weeds) and size greater
	 * or equal than 2.
	 * 
	 * @param crop
	 *            reference to ICropTile
