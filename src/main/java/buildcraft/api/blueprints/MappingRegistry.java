...
public class MappingRegistry {
	...
	public Item getItemForId(int id) throws MappingNotFoundException {
		if (id >= idToItem.size()) {
			throw new MappingNotFoundException("no item mapping at position "
					+ id);
		}
	...
	public Block getBlockForId(int id) throws MappingNotFoundException {
		if (id >= idToBlock.size()) {
			throw new MappingNotFoundException("no block mapping at position "
					+ id);
		}
	...
	public Class<? extends Entity> getEntityForId(int id)
		...
		if (id >= idToEntity.size()) {
			throw new MappingNotFoundException("no entity mapping at position "
					+ id);
		}
	...
	private boolean isStackLayout(NBTTagCompound nbt) {
		return nbt.hasKey("id") && nbt.hasKey("Count") && nbt.hasKey("Damage")
			...
			if (nbt.getTag(key) instanceof NBTTagList) {
				...
				if (list.func_150303_d() == Constants.NBT.TAG_COMPOUND) {
					for (int i = 0; i < list.tagCount(); ++i) {
						scanAndTranslateStacksToRegistry(list
								.getCompoundTagAt(i));
					}
				}
			}
			...
			if (nbt.getTag(key) instanceof NBTTagList) {
				...
				if (list.func_150303_d() == Constants.NBT.TAG_COMPOUND) {
					for (int i = list.tagCount() - 1; i >= 0; --i) {
						try {
							scanAndTranslateStacksToWorld(list
									.getCompoundTagAt(i));
						} catch (MappingNotFoundException e) {
							list.removeTag(i);
						}
