 ...
 * like to easily deploy structures that are written in blueprints. It does not
 * offer control on material that needs to get in, or how the structure is
 * deployed, but allows to create contents of a blueprint in one cycle. Note
 * that these functionalities will only work if BuildCraft is installed.
 */
public abstract class BlueprintDeployer {
	...
	public abstract void deployBlueprintFromFileStream(World world, int x,
			int y, int z, ForgeDirection dir, byte[] data);
