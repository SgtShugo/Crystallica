 ...
 * blueprint can be used. A same schematic class can have different permissions
 * depending on its contents. It's particularly useful when fixing a schematic,
 * if blueprints that saved the previous version should not be built because of
 * a bug (such as dupe bug on inventories).
 */
public enum BuildingPermission {
	...
	/**
	 * This blueprints containing this schematic can only be used in creative.
	 * Maybe the block can't be crafted in survival in the first place, or the
	 * content of the schematic is known to have dupe bugs.
	 */
	CREATIVE_ONLY,
