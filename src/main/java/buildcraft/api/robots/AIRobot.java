...
public class AIRobot {
	...
	public final void loadFromNBT(NBTTagCompound nbt) {
		...
		if (nbt.hasKey("delegateAI")) {
			...
			try {
				delegateAI = (AIRobot) Class.forName(sub.getString("class"))
						.getConstructor(EntityRobotBase.class)
						.newInstance(robot);
	...
	public static AIRobot loadAI(NBTTagCompound nbt, EntityRobotBase robot) {
		...
		try {
			ai = (AIRobot) Class.forName(nbt.getString("class"))
					.getConstructor(EntityRobotBase.class).newInstance(robot);
			ai.loadFromNBT(nbt);
		} catch (Throwable e) {
			e.printStackTrace();
		}
